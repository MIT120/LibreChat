...
export class SchemaRegistry {
    ...
    private zodToJsonSchema(schema: ZodSchema): Record<string, any> {
        ...
        if (schema instanceof z.ZodObject) {
            ...
            Object.entries(schema.shape).forEach(([key, value]) => {
                const zodSchema = value as ZodSchema;
                properties[key] = this.zodTypeToJsonSchema(zodSchema);

                // Check if field is required
                // Fields are NOT required if they are:
                // 1. ZodOptional (optional fields)
                // 2. Have a transform with default value (like our auto-generated IDs)
                // 3. Have a default value
                const isOptional = zodSchema instanceof z.ZodOptional;

                // Check for transforms in optional fields (ZodOptional wraps the actual schema)
                let hasTransform = false;
                if (isOptional) {
                    const innerSchema = (zodSchema as any)._def?.innerType;
                    hasTransform = innerSchema?._def?.transform !== undefined;
                } else {
                    hasTransform = (zodSchema as any)._def?.transform !== undefined;
                }

                const hasDefault = (zodSchema as any)._def?.defaultValue !== undefined;

                // Special handling for our auto-generated ID fields - these should not be required
                const isAutoGeneratedId = (key === 'authorId' || key === 'conversationId') && (isOptional || hasTransform);

                if (!isOptional && !hasTransform && !hasDefault && !isAutoGeneratedId) {
                    required.push(key);
                }

                // Add description for auto-generated fields
                if (isAutoGeneratedId) {
                    const currentDesc = properties[key].description || '';
                    properties[key].description = currentDesc +
                        (currentDesc ? ' ' : '') + '(optional - will be auto-generated if not provided)';
                }
            });
    ...
    private zodTypeToJsonSchema(zodType: ZodSchema): Record<string, any> {
        ...
        if (zodType instanceof z.ZodOptional) {
            const unwrapped = this.zodTypeToJsonSchema(zodType.unwrap());
            // Mark the field as having a default if it has a transform
            const innerSchema = (zodType as any)._def?.innerType;
            if (innerSchema?._def?.transform) {
                unwrapped.description = (unwrapped.description || '') + ' (optional - auto-generated if not provided)';
            }
            return unwrapped;
        }
        ...
        operations.forEach(operation => {
            ...
            switch (operation) {
                ...
                case 'get':
                    ...
                    break;
                case 'list':
                    // Use specific schema if available, otherwise fall back to generic
                    const specificListSchema = (EntitySchemas as any)[`list${entityName}s`] || (EntitySchemas as any)[`list${entityName}`];
                    schema = specificListSchema || EntitySchemas.listBooks;
