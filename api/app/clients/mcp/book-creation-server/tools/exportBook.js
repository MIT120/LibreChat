...
const { validateExportBookParams, sanitizeAndValidate, formatValidationErrors } = require('../utils/validators');
...
const exportBookTool = {
  name: 'export_book',
  description: 'Export a completed book in various formats (markdown, html, txt, json). Generates downloadable content with customizable formatting options.',
  inputSchema: {
    type: 'object',
    properties: {
      bookId: {
  ...
  async execute(params, context) {
    try {
      ...
      const contentPreview = exportResult.content.length > 500 
      ...
      return {
        ...
        message: `Book "${exportResult.title}" successfully exported as ${exportResult.format.toUpperCase()} format. File size: ${this.formatFileSize(exportResult.metadata.fileSize)}.`
      };
      ...
      if (error.message.includes('not completed')) {
        return {
          ...
          error: {
            ...
            message: 'Only completed books can be exported',
            details: {
              suggestion: 'Complete all chapters before attempting export'
            }
          }
        };
      }
      ...
      if (error.message.includes('format not supported')) {
        return {
          ...
          error: {
            ...
            details: {
              supportedFormats: ['markdown', 'html', 'txt', 'json']
            }
          }
        };
      }
  ...
  calculateExportStatistics(exportResult) {
    ...
    return {
      ...
      processingTime: Date.now() - (exportResult.metadata.exportedAt ? new Date(exportResult.metadata.exportedAt).getTime() : Date.now())
    };
  },
  ...
  countJsonProperties(obj) {
    ...
    return Object.keys(obj).length + Object.values(obj).reduce((sum, value) => sum + this.countJsonProperties(value), 0);
  },
