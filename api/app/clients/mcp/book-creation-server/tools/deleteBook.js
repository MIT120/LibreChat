...
const deleteBookTool = {
  name: 'delete_book',
  description:
    'Delete a book project and all associated data. This action is irreversible and requires explicit confirmation.',
  inputSchema: {
    type: 'object',
    properties: {
      bookId: {
    ...
    try {
      ...
      if (sanitizedParams.confirm !== true) {
        return {
          ...
          error: {
            ...
            message:
              'Explicit confirmation required to delete book. Set confirm parameter to true.',
            details: {
      ...
      try {
        ...
        if (sanitizedParams.backupFirst) {
          backupData = await this.createBookBackup(
            bookService,
            userId,
        ...
        console.warn(
          '[DeleteBookTool] Could not retrieve book info before deletion:',
          error.message,
        );
      }
      ...
      if (!deletionResult) {
        return {
          ...
          error: {
            ...
            details: {
              bookId: sanitizedParams.bookId,
              possibleReasons: ['Book not found', 'Insufficient permissions', 'Database error'],
            },
          },
        };
      }
      ...
      if (backupData) {
        responseData.backup = {
          ...
          instructions:
            'Backup data is included in this response. Save it if you need to restore the book later.',
        };
      }
      ...
      return {
        success: true,
        data: responseData,
        message: `Book "${bookInfo?.title || sanitizedParams.bookId}" has been permanently deleted${backupData ? ' (backup created)' : ''
          }. All associated chapters and data have been removed.`,
        warnings: [
          'This action cannot be undone',
          'All chapter content has been permanently removed',
          backupData
            ? 'Backup data is included in this response - save it if needed'
  ...
  calculateBookAge(createdAt) {
    ...
    if (diffDays === 0) {
      return {
        ...
        hours: diffHours,
        description:
          diffHours === 0 ? 'Less than an hour' : `${diffHours} hour${diffHours !== 1 ? 's' : ''}`,
      };
    }
  ...
  generatePostDeletionRecommendations(bookInfo, reason) {
    ...
    switch (reason) {
      ...
      case 'test_project':
        recommendations.push({
          type: 'learning',
          message:
            "Now that you're familiar with the system, consider starting your main book project",
          action: 'create_book_project',
          priority: 'high',
        });
        break;
    ...
    if (bookInfo?.progress?.completionPercentage > 50) {
      recommendations.push({
        type: 'warning',
        message:
          'You deleted a book that was more than 50% complete. Consider if this was intentional.',
        priority: 'high',
      });
    }
